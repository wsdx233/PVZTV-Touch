...
{
	while(!FLfile.write(gURI,message,"append"))
	{
		alert("Write failed.");
	}
}
...
function WriteReanimFrame(sampleFrame, frameArray, prev, URI, elementIndex)
{
	...
	*/
	...
	else if (baseElement.elementType == "text")
	{
		...
		//write("<font>FONT_" + fontName + "</font>\n");
		//write("<text>" + XmlEncode(tRuns[0].characters) + "</text>\n");
	...
	else
	{
		...
		{
			if( baseElement.libraryItem.name.indexOf(".psd") == -1 )
			{
				imageName = "IMAGE_REANIM_" + baseElement.libraryItem.name.replace(/\..*/g, '').toUpperCase();
			}
			else
			{
				pathindex  = baseElement.libraryItem.name.lastIndexOf("/");
				if( pathindex == -1 )
				{
					imageName = "IMAGE_REANIM_" + baseElement.libraryItem.name.replace(/\..*/g, '').toUpperCase();
				}
				...
				{
					imageName = "IMAGE_REANIM_LAYER_" + baseElement.libraryItem.name.substr(pathindex+1).toUpperCase();
				}
			}
//			fl.trace("Item name: " + imageName );
	...
	{
		write("<kx>" + stringDegreesKX + "</kx>");
	}
	
	if (prev.prevKY != stringDegreesKY)
	{
		write("<ky>" + stringDegreesKY + "</ky>");
	}
				
	if (prev.prevSX != stringSX)
	{
		write("<sx>" + stringSX + "</sx>");
	}
	
	if (prev.prevSY != stringSY)
	{
		write("<sy>" + stringSY + "</sy>");
	}
	...
	if (prev.prevText != stringText)
	{
		...
		{
			write("<text>" + XmlEncode(stringText) + "</text>");
		}			
	}
...
if (FLfile.exists(gURI))
{
	var attr = FLfile.getAttributes(gURI);
